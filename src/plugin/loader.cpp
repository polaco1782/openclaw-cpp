...
PluginLoader::PluginLoader() {
    ...
    const char* env_path = std::getenv("OPENCLAW_PLUGIN_PATH");
    if (env_path) {
        ...
        while ((end = paths.find(':', start)) != std::string::npos) {
            if (end > start) {
                search_paths_.insert(search_paths_.begin(), paths.substr(start, end - start));
            }
            start = end + 1;
        }
        if (start < paths.length()) {
            search_paths_.insert(search_paths_.begin(), paths.substr(start));
        }
    }
}
...
int PluginLoader::load_from_config(const Config& config) {
    ...
    if (plugins_config.is_array()) {
        ...
        const std::vector<Json>& arr = plugins_config.as_array();
        ...
        const std::map<std::string, Json>& obj = plugins_config.as_object();
        for (std::map<std::string, Json>::const_iterator it = obj.begin();
             it != obj.end(); ++it) {
            // Check if plugin is enabled
            const Json& plugin_cfg = it->second;
            if (plugin_cfg.is_object()) {
                bool enabled = true;
                if (plugin_cfg.has("enabled")) {
                    enabled = plugin_cfg["enabled"].as_bool(true);
                }
                if (!enabled) {
                    LOG_DEBUG("Plugin %s disabled in config", it->first.c_str());
                    continue;
                }
            }
    ...
    // Also check plugins_dir setting
    std::string plugins_dir = config.get_string("plugins_dir", "");
    if (!plugins_dir.empty()) {
        count += load_dir(plugins_dir);
    }
